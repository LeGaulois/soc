...
from importation import XMLHote
...
def importXML(request):
    '''
    Vue permettant l'ajout en base d'hôtes présents dans un fichier XML
    '''
    if request.method == 'POST':

        form = formUploadXML(request.POST, request.FILES)
        	
        #On verifie si les donnees recu sont valides
        if form.is_valid():
            try:
                xml=XMLHote(request.FILES['file'])
                xml.importer()
                return redirect ('serveurs:liste')
            except ErreurImport as e: 
                erreurs=e.getData()
                return render(request, 'serveurs/importation.html', locals())
        else:
            return render(request, 'serveurs/importation.html', locals())
...
def ajout(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            environnement=form.cleaned_data['environnement']
