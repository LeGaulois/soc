...
class formEditMachine(forms.Form):
	...
	def __init__(self,*args,**kwargs):
		...
		CHOIX_TYPE=[ 
			(None,'A définir'),
			('equipement-reseau','Equipement réseau'),
			('serveur','Serveur'),
		...
		self.fields['type_machine']=forms.CharField(label='Type machine',initial=type_machine,widget=forms.Select( choices=CHOIX_TYPE),required=False)
		self.fields['environnement']=forms.CharField(label='Environnement',initial=environnement,widget=forms.Select( choices=CHOIX_ENVIRONNEMENT),required=False)
		...
		self.fields['localisation']=forms.CharField(label='Localisation',initial=localisation,widget=forms.Select( choices=CHOIX_LOCALISATION),required=False)
	...
	def clean_adresse(self):
		...
		if(valideIP(adresse)==False):
			raise forms.ValidationError('adresse IP non valide')

		if self.mode=='ajout':
			ADRESSES_EXISTANTES=[]

			for k  in range(len(self.adresses)):
				ADRESSES_EXISTANTES.append(str(self.adresses[k]['ip']))

			if adresse in ADRESSES_EXISTANTES:
				raise forms.ValidationError('IP déjà pésente')

		else:
			if self.adresse!=self.cleaned_data['adresse']:
				raise forms.ValidationError('L"'"IP renseignée est différente de l"'"adresse appelé')
	...
	def clean_criticite(self):
		...
		liste_criticite=['extreme','haute','moyenne','faible','nc']
	...
	def clean_environnement(self):
		...
		liste_environnement=[None,'production','pre-production','recette']
	...
	def clean_localisation(self):
		...
		liste_localisation=['sarcelles','valence','nc']
	...
	def clean_type_machine(self):
		...
		liste_type=['equipement-reseau','serveur','vm','Machine Virtuelle','appliance','nc']
