...
def getRapport(request,id_scan,type_scan,type_rapport):
	...
	chemin='./html/django/soc/rapports/rapports/'
...
WHERE scans_status.id=%s LIMIT 1''',[id_scan])
		...
		id_scan_type=id_dict[0]['id_scan_manuel']
...
WHERE scans_status.id=%s LIMIT 1''',[id_scan])
	...
	if type_rapport=='nessus':
		liste_fichier=glob.glob(chemin+str(id_scan_type)+"__*_nessus.pdf")

		if len(liste_fichier)==0:
			if type_scan=='manuel':
				return redirect('scans:parametresScanManuel',int(id_scan))
			else:
				return redirect('scans:historiqueScanPlannifie',int(id_scan_type))

		nom_fichier=liste_fichier[0].split('/')[-1]

	else:
		
		liste_fichier=glob.glob(chemin+str(id_scan_type)+"__*_evolution.pdf")
