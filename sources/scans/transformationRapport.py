...
def parserNmapXml(fichiercsv,date_scan,scan_id):
	'''
	...
	for elem in liste_args:
		if re.search('-pT',elem)!=None:
			options=elem.split(',U:')
			ports_tcp_scannes=options[0].split('-pT:')[1].split(',')

			try:
				ports_udp_scannes=options[1].split(',')
		...
		elif re.search('-pU',elem)!=None:
			ports_udp_scannes=elem.split('-pU:')[1].split(',')
			break
